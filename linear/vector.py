...
class Vector(object):
    ...
    def times_scalar(self, c):
        new_coordinates = [Decimal(x) * Decimal(c) for x in self.coordinates]
        return Vector(new_coordinates)

    def magnitude(self):
        return Decimal(math.sqrt(sum([Decimal(x)**2 for x in self.coordinates])))
    
    def normalized(self):
        try:
            mat = self.magnitude()
            return self.times_scalar(Decimal(1)/mat)
    ...
    def is_parallel_to(self, v):
        return ( self.is_zero() or
            ...
            MyDecimal.is_near_zero(self.angle_with(v) - Decimal(math.pi)) ) #接近pi
